...
main_file_path_csv = os.getenv('FILE_PATH_CSV_MONTH')
...
train_arrivals_csv_headers = ['Station_ID', 'Stop_ID', 'Station_Name', 'Destination', 'Route',
                              'Run_Number', 'Prediction_Time', 'Arrival_Time', 'Headway', 'Time_Of_Week', 'Time_Of_Day']
    ...
    for item in data:
        ...
        with open(csv_file_path, 'a', newline='', encoding='utf8') as csvfile:
            ...
            writer_object.writerow({'Station_ID': item["train_arrivals[Station_ID]"], 'Stop_ID': item["train_arrivals[Stop_ID]"],
                                    ...
                                    'Arrival_Time': item["train_arrivals[Arrival_Time_Combined]"], 'Headway': item["train_arrivals[Headway]"], 'Time_Of_Week': item["train_arrivals[Time of Week]"], 'Time_Of_Day': item["train_arrivals[Time Of Day]"]})
...
while remaining > 0:
    last_month = get_date("longmonth", remaining)
